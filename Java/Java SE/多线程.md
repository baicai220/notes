...
* **线程（thread）**：进程可进一步细化为线程，是程序内部的`一条执行路径`。一个进程中至少有一个线程。
  - 一个进程同一时间若`并行`执行多个线程，就是支持img/多线程的。
  - 线程作为`CPU调度和执行的最小单位`。
...
Java通过继承Thread类来**创建**并**启动img/多线程**的步骤如下：
...
> 1. 如果自己手动调用run()方法，那么就只是普通方法，没有启动img/多线程模式。
>
> 2. run()方法由JVM调用，什么时候调用，执行的过程控制都有操作系统的CPU调度决定。
>
> 3. 想要启动img/多线程，必须调用start()方法。
>
...
实际上，所有的img/多线程代码都是通过运行Thread的start()方法来运行的。因此，不管是继承Thread类还是实现
Runnable接口来实现img/多线程，最终还是通过Thread的对象的API来控制线程的，熟悉Thread类的API是进行img/多线程编程的基础。
...
```java
/*
 * 创建img/多线程的方式三：实现Callable （jdk5.0新增的）
 */
...
Java中提供了同步机制(synchronized)来解决img/多线程并发访问一个资源的安全性问题。
...
public class LazyOne {
	...
	5. 指令重排导致问题并不是因为初始化对象会改变内存地址，而是在img/多线程环境下，由于重排可能会让其他线程提前看到一个指向未初始化对象的引用
