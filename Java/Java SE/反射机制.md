...
Reflection（反射）是被视为`动态语言`的关键，img/反射机制允许程序在`运行期间`借助于Reflection API取得任何类的内部信息，并能直接操作任意对象的内部属性及方法。
...
+ **缺点：**
  ...
  - 反射的`性能较低`。
    - img/反射机制主要应用在对灵活性和扩展性要求很高的系统框架上
...
这是img/反射机制应用最多的地方。创建运行时类的对象有两种方式：
...
在img/反射机制中，可以直接通过Field类操作类中的属性，通过Field类提供的set()和get()方法就可以完成设置和取得属性内容的操作。
